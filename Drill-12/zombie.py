...
class Zombie:
    ...
    def find_ball(self):
        ...
        for ball in balls:
            ...
            if ball_distance <= distance:
                ball_distance = distance
                real_ball = ball

        if ball_distance < (PIXEL_PER_METER * 5)**2:
            self.dir = math.atan2(real_ball.y - self.y, real_ball.x - self.x)
            print("찾았다")
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        hp_big_node = LeafNode("HP Big", self.Hp_big)
        find_player_node = LeafNode("Find Player", self.find_player)
        move_to_player_node = LeafNode("Move to Player", self.move_to_player)
        hp_small_node = LeafNode("HP Small", self.Hp_small)
        ...
        chase_player_node.add_children(hp_big_node, find_player_node, move_to_player_node)
    ...
    def get_bb(self):
        return self.x - 40, self.y - 50, self.x + 40, self.y + 40
    ...
    def draw(self):
        self.font.draw(self.x - 60, self.y + 50, '(HP: %3.0f)' % self.Hp, (0, 0, 255))
